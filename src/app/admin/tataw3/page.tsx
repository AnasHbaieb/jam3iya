...
const AdminVolunteerApplicationsPage = () => {
  ...
  const exportToExcel = () => {
    ...
    const ws = XLSX.utils.json_to_sheet(applications.map(app => ({
      ...
      interestAreas: app.interestAreas.join(', '), // دمج المجالات كـ string
    })));
    
    // Format the date column
    const wsRows1: CellObject[][] = (ws['!rows'] as CellObject[][]);
    if (wsRows1) {
      applications.forEach((app, index) => {
        const dateCell = wsRows1[index + 1]?.[10]; // createdAt is the 11th column (0-based index) after adding ID
        if (dateCell) {
          dateCell.t = 'd'; // Mark as date
          dateCell.z = 'yyyy-mm-dd'; // Set date format
        }
      });
    }
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'تطوع');
    
    XLSX.writeFile(wb, 'تطوع_التطبيقات.xlsx');
  };
